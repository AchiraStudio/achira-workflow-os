...
try:
    from shared_utils import Colors, print_header, print_success, print_error, print_warning  # type: ignore
except ImportError:
    ...
    def print_success(t): print(f"{Colors.GREEN}✅ {t}{Colors.ENDC}")
    def print_error(t): print(f"{Colors.RED}❌ {t}{Colors.ENDC}")
    def print_warning(t): print(f"{Colors.YELLOW}⚠️  {t}{Colors.ENDC}")

DATA_DIR = Path(__file__).parent.parent / "data"

def find_row_by_keyword(filepath: Path, keyword: str) -> Optional[Dict[str, str]]:
...
def generate_css(color_row: Optional[Dict[str, str]], typo_row: Optional[Dict[str, str]]) -> str:
...
def main():
    ...
    print_header(f"✨ Generating Theme for: '{args.query}'")
    ...
    if color_row:
        print(f"  🎨 Color Palette: {color_row['Product Type']}")
    if typo_row:
        print(f"  🔡 Typography: {typo_row['Font Pairing Name']}")
